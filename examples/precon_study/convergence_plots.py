...
with open(logfile, 'r') as file:
    ...
    for line in file:
        ...
        if mesh_match:
            current_mesh_size = mesh_match.group(1)  # Aktuelle Mesh-Größe setzen
            if current_mesh_size not in data:
                data[current_mesh_size] = ([], [])  # Leere Listen für Iterationen und Residuen

        if current_mesh_size:  # Nur wenn eine Mesh-Größe gesetzt ist
            residual_match = re.search(r"^\s*(\d+)\s+KSP.*\|\|r\(i\)\|\|/.*\|\|b\|\|\s+([\d\.eE+-]+)", line)
            if residual_match:
                # Iterationsnummer und relatives Residuum speichern
                iterations, relative_residues = data[current_mesh_size]
                iterations.append(int(residual_match.group(1)))  # Iterationsnummer
                relative_residues.append(float(residual_match.group(2)))  # Relatives Residuum
...
for mesh_size, (iterations, relative_residues) in data.items():
    ...
    plt.plot(iterations, relative_residues, marker='o', linestyle='-', label=f'Mesh: {mesh_name}')
...
output_file = os.path.join(output_dir, f"convergence_plot_{output_dir_name}.png")

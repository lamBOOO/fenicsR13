...
def extract_log_info(logfile_path):
    ...
    solved_times = []
    iteration_values = []
    last_iteration = None  # Hält den zuletzt gefundenen Iterationswert im aktuellen Block

    with open(logfile_path, 'r') as logfile:
        for line in logfile:
            # Suche in jeder Zeile nach einem Iterationswert
            iteration_match = re.search(r'(\d+)\s+KSP (?:preconditioned|unpreconditioned) resid norm', line, re.IGNORECASE)
            if iteration_match:
                # Speichere den aktuellen Iterationswert als "letzten" im Block
                last_iteration = int(iteration_match.group(1))
            
            # Suche nach einem "Finished solve:"-Eintrag
            solve_match = re.search(r'Finished solve:\s*(\d+\.?\d*)', line, re.IGNORECASE)
            if solve_match:
                solved_time = float(solve_match.group(1))
                solved_times.append(solved_time)
                # Für den abgeschlossenen Block: Wenn ein Iterationswert vorhanden ist, diesen übernehmen;
                # sonst None eintragen.
                iteration_values.append(last_iteration)
                # Setze last_iteration für den nächsten Block zurück
                last_iteration = None
        ...
        for file in files:
            if file.endswith('.log'):
                ...
                if solved_times and iterations:
                    # Da beide Listen pro Block einen Eintrag enthalten, können wir zip verwenden.
...
def write_to_csv(output_path, log_data):
    ...
    with open(output_path, 'w', newline='') as csvfile:
        ...
        # Für jeden Ordner wird eine Zeile geschrieben, in der alle Werte als kommaseparierter String stehen.
...
def update_and_run_simulations():
    ...
    original_yaml = "../../tests/square_manufactured_solution/inputs/square_manufactured_solution_2d_r13.yml"
    output_dir = "../../examples/precon_study/Simulation_Output_v1"  # Verzeichnis für Logs und Plots

    # Optionen für das Kartesische Produkt
    ksp_types = ["gmres", "fgmres"]#, "tfqmr"]
    pc_types = ["fieldsplit"]
    fieldsplit_types = ["schur"]#, "additive", "multiplicative", "symmetric_multiplicative"]
    schur_fact_types = ["lower"]#,"full", "diag", "upper"]
    fieldsplit_0_types = ["hypre"]#,"jacobi", "bjacobi", "sor","lu", "qr", "shell", "eisenstat", "ilu", "icc", "asm", 
    ...
    for ksp in ksp_types:
        for pc in pc_types:
            if pc == "fieldsplit":
                ...
                # Für andere pc-Typen (falls vorhanden) wird das volle Produkt verwendet.
                for f, s in itertools.product(fieldsplit_types, schur_fact_types):
                    combinations.append((ksp, pc, f, s, f0, f1))

    for idx, (ksp_type, pc_type, fieldsplit_type, schur_fact_type, fieldsplit_0_type, fieldsplit_1_type) in enumerate(combinations):
        # Erstellen des spezifischen Output-Ordners im precon_study-Verzeichnis
        if schur_fact_type is None:
            output_folder_name = f"{ksp_type}_{fieldsplit_type}_{fieldsplit_0_type}_{fieldsplit_1_type}"
        else:
            output_folder_name = f"{ksp_type}_{fieldsplit_type}_{schur_fact_type}_{fieldsplit_0_type}_{fieldsplit_1_type}"
        ...
        if pc_type == "fieldsplit":
            if fieldsplit_type == "schur":
                ...
                "petsc_options.pc_fieldsplit_schur_fact_type": schur_fact_type,
        ...
        print(f"Updating YAML with the following options: {json.dumps(updates, indent=2)}")

        output_file_name = f"config_update_{idx + 1}.yml"
        yaml_output_file_path = os.path.join('../../tests/square_manufactured_solution/inputs/', output_file_name)

        subprocess.run(["python3", "../../examples/precon_study/update_yml.py",
                        original_yaml, yaml_output_file_path, json.dumps(updates)], check=True)

        log_file_name = f"update_{idx + 1}.log"
        log_file_path = os.path.join(output_folder, log_file_name)

        result = subprocess.run(["fenicsR13", f"../../tests/square_manufactured_solution/inputs/{output_file_name}"],
                                stdout=open(log_file_path, "w"), stderr=subprocess.STDOUT)
        ...
        plot_result = subprocess.run(["python3", "../../examples/precon_study/convergence_plots.py", log_file_path])
